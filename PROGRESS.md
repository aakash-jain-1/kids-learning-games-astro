...
> **▶ Resume here next session:** the **migration is complete (13/13)** as of 2026-05-08, **Track 1 of post-migration polish is complete — 11 of 11 non-story games wired** as of 2026-05-11, **Track 2 (Playwright smoke tests) bootstrapped same day** with **47 tests passing 47/47** end-to-end against the live GitHub Pages deploy in 22.2 s wall-clock and CI verified green twice on `main`, and **Track 3 (Option C — unified `DeckLayout`) closed 2026-05-11 as NO-GO** with full ADR-style rationale captured under "Rough order of payoff → 5" below. Layout audit + page audit + CSS-bundle audit produced five categories of evidence against consolidation (different detail-payload shapes, different filter bars, different state shapes, different viewport contracts, no Vite tree-shaking on conditional CSS imports) and one productive smaller win: `<GameControls />` (`src/components/GameControls.astro`, 29 LoC) consolidated the byte-identical Quiz/Stats/Settings ctrl-row that was duplicated across all 13 pages; optional `quiz?: boolean` prop (defaults `true`; Woodcutter passes `false`); rendered DOM byte-identical so the 47 Playwright assertions against `#btnQuiz` / `#btnStats` / `#btnSettings` continue to pass. Net source delta: **~−9 LoC overall** (−38 page lines + 29 new component lines); the real win is consolidating ~52 lines of duplicated markup into one source-of-truth component so future button changes become 1-line edits. README + Playwright tests CI badges added for at-a-glance status. **Next track:** **Track 4 — cut-over plan**, unblocked: migrate the live `kids-learning-games` repo to serve the Astro `dist/` build with a SW handoff strategy so existing PWA installs gracefully transition to the new SW (vanilla SW unregisters → new Workbox SW registers → cache invalidation tested across Chrome/Safari/Firefox; manifest scope and start_url updated). **Other queued follow-ups (smaller scope):** (T2.1) promote Playwright from soft to hard deploy gate by adding `needs: test` to the `build` job in `.github/workflows/deploy.yml` (one-line tweak; doc says wait until the suite has run cleanly across ~5 normal-day commits in CI without flaking — currently at 2 clean runs, 3 to go); (T6) consider whether the Stats panel (currently `alert(…)` aggregations across every game) deserves a dedicated `/stats` page or per-page Stats modal — Playwright now locks the existing alert-shape behaviour in, so this is safe to refactor when ready. Build is clean: 14 pages built in 7.49 s, `npm run check` 0 errors / 0 warnings / 0 hints across **44 Astro files** (was 43 — `GameControls.astro` is the +1), all chunk-dedup invariants still verified at the bundle level (quiz **13-way**, progress 8-way, fluent 6-way, achievements 13-way, layout pre-paint 3-way).
