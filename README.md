...
A proof-of-concept migration of the [kids-learning-games](../kids-learning-games) vanilla HTML/CSS/JS PWA to **Astro + TypeScript + @vite-pwa/astro (Workbox)**. **Six games ported end-to-end**, across **two shared layouts** that together cover every non-story game in the vanilla repo:
...
- `GridLayout.astro` — **foundational-set games** (scan a fixed chart, tap to hear). Hosts Alphabets and Numbers today; Colors, Shapes, Animals, Birds, and Hindi all land here next.
...
- **Two shared layouts, six games, six themes**:
  ...
  - `GridLayout.astro` hosts Alphabets (image detail) and Numbers (CSS count-objects detail) — a foundational-set tile chart with filter + inline detail card + per-tile learned-state tracking via `kids_progress_v1:<gameId>`. Two deck variants in `grid.css`: `--capped` for medium decks, `--numbers` 5-column fixed for small decks.
...
- **Per-game learning state**: `kids_progress_v1:<gameId>` LocalStorage key with a JSON array of learned-item ids. Alphabets and Numbers both consume the shared `src/lib/progress.ts` helper (`loadLearned` / `saveLearned` / `clearLearned`), extracted on the second consumer per the "two-consumers triggers a refactor" rule.
...
│   │   ├── progress.ts           # kids_progress_v1:<gameId> store (alphabets, numbers)
...
- The remaining 5 foundational-set games (Colors, Shapes, Animals, Birds, Hindi) — all scheduled to land on `GridLayout`; each port is ~35–50 lines of `--gl-*` theme tokens + ~200–300 lines of page + typed data. Per-game layout decisions and gotchas are tracked in [`PROGRESS.md → Per-game layout decisions`](./PROGRESS.md#per-game-layout-decisions-for-the-7-pending-ports).
...
- Full nav bar (nav lists only the 6 ported games + home; other links would 404 until those pages are ported).
...
- Quiz mode and full Stats modal — the top-bar buttons exist as `alert(…)` stubs in all 6 ported games. Settings storage is wired. `kids_progress_v1:<gameId>` is a real shipping pattern: both alphabets and numbers consume `src/lib/progress.ts` (extracted now that two games need it). The Stats modal can read from it directly when wired.
- Full test suite.
...
| Reused layout lines | ~0 (copy-pasted per game) | ~140 `CardMachineLayout.astro` (4 games) + ~150 `GridLayout.astro` (alphabets + numbers + 5 upcoming) — two shells cover all 11 non-story games |
| Shared CSS | Duplicated 4× inline (~450 lines each) | `card-machine.css` (~1050 lines, 4 games) + `grid.css` (~720 lines, 7 games) + shared primitives in `global.css` |
| Shared JS util lines | Duplicated per game inline | 1 copy under `src/lib/` |
| Settings storage | `flashcards_settings` vs `darkMode` vs `solar_system_settings` vs `weather_settings` vs `alphabet_learned` vs `numbers_learned` | Single `kids_settings_v1` + per-game `kids_progress_v1:<gameId>` (alphabets, numbers; shared `src/lib/progress.ts` helper) |
...
| Client JS bundle, gzipped | Inline, unminified | flashcards **11.28 KB**, weather **3.34 KB**, dinosaurs **3.02 KB**, alphabets **2.96 KB**, solar-system **2.66 KB**, numbers **2.08 KB** |
