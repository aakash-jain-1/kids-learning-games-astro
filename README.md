...
│   │   └── stats-registry.ts     # T6 — single source of truth for the parent-facing /stats page; one StatsRegistryEntry per game across 4 families (preschool-math / story / card-set / card-pure) with read()/clear()/hasData() — adding a 17th game = one entry edit
│   ├── layouts/
...
**Nine Playwright smoke suites** covering three categories:
...
3. **Infra-hardening** — `tests/not-found.spec.ts` (T7, the friendly 404
   ...
   T6's tests assert against the `STATS_REGISTRY` shape (16 cards, 4
   family sections) — adding a 17th game means a one-entry edit in the
   registry plus a one-line append to `EXPECTED_GAME_IDS` in the spec.
