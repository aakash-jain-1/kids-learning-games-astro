...
- ~~Cut-over plan — the live `kids-learning-games` repo still serves the vanilla static HTML pages. Migrating it to serve the Astro `dist/` build (with a SW handoff strategy so existing PWA installs gracefully transition to the new SW) is the final piece, intentionally postponed until after this POC's content stabilised.~~ **Phase 1 (decision + ADR) + Phase 2 (groundwork code changes) closed 2026-05-12** as Track 4 of the post-migration polish. **Decision: Option A — Astro takes over the vanilla URL `/kids-learning-games/`, vanilla repo becomes the dist host, two-repo source split kept for now.** Phase 2 shipped against the staging URL with three changes: (a) SW source rename `src/sw.ts` → `src/service-worker.ts` (output URL `<base>/service-worker.js` — matches vanilla's filename so cut-over is a transparent same-URL byte swap; existing vanilla PWA installs auto-migrate via the standard browser SW update flow, no special unregister dance); (b) `astro.config.mjs` `redirects` block emits build-time `<meta http-equiv="refresh">` HTMLs at the four divergent vanilla URLs (`alphabet-game.html` singular → `alphabets-game.html`, `birds.html` → `birds-game.html`, `daily-routines.html` → `daily-routines-game.html`, `hindi-alphabets.html` → `hindi-game.html`) so vanilla bookmarks continue to land on the right page; (c) offline-fallback URL bug fix in the SW (was hardcoded `/kids-learning-games/offline.html` — wrong base AND wrong extension because `@vite-pwa/astro` strips `.html` on HTML files; fixed to `'offline'` resolved at SW install via `new URL('offline', self.location.href)`, base-portable through Phase 3). **Phase 3 — URL flip + cross-repo deploy queued for next session, requires explicit user OK before any vanilla-repo writes.** See `PROGRESS.md` → "Rough order of payoff → 6" for the full ADR.
