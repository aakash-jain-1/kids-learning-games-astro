...
- Wire the real Stats + Quiz modals across **all** ported games — *in progress (1 of 11 non-story games wired as of 2026-05-08)*. **Dinosaurs** went first on `mountQuiz` from `src/lib/quiz.ts` (5-question quiz on the deck contents + Stats panel reading `quiz.getState()` aggregations). The shared `card-machine.css` now ships a `.cm-quiz-overlay` + `.cm-quiz-card` modal shell + 4-theme `--cm-quiz-*` palette, so the remaining 3 card-machine games (Flashcards, Solar System, Weather) only need a `QUIZ` array + page wiring. The 7 grid games will follow once the card-machine sweep lands; that's also the third-consumer trigger for extracting the inner `.quiz-question` / `.quiz-opt` / `.quiz-result-*` selectors to a shared `src/styles/quiz-modal.css`.
...
| Client JS bundle, gzipped | Inline, unminified | flashcards **11.30 KB**, **routines ~4.10 KB** (10 scenes' inline `artHtml` + 8-question quiz), **dinosaurs 3.71 KB** *(post-quiz-wiring; +0.67 KB over pre-quiz baseline for the Quiz modal handlers)*, hindi **~5.25 KB**, weather **3.36 KB**, **animals 3.31 KB**, alphabets **2.98 KB**, solar-system **2.68 KB**, **birds 2.55 KB**, colors **2.27 KB**, shapes **2.13 KB**, numbers **2.09 KB**, **woodcutter 1.46 KB** (smallest — pure pre-rendered scene art + tiny quiz wiring); shared `quiz.ts` chunk **0.98 KB** now dedups **3-way** across routines + woodcutter + dinosaurs |
