...
- **State (2026-05-20): all 13 vanilla games ported and live (migration
  complete since 2026-05-08), plus 4 new feature-driven games — Counting
  ...
  completes the cardinality triad), and Pattern Sequences (preschool-math
  pattern recognition for ages 3–4, shipped 2026-05-20 — same session as
  T-retention, sister to the cardinality triad). Total 17 games** at
...
- **Three shared layouts** in production:
  ...
  - `StoryLayout` (5 games — Daily Routines paginated, Honest Woodcutter,
...
- **NEXT SESSION CANDIDATES (snapshot 2026-05-20, latest — UPDATED post T-patterns ship)**: with the cardinality triad shipped + Pattern Sequences (option 2) shipped + UX-quality work (T-extra + triad-extension) closed + retention instrumentation (option 4) shipped — all in this same session — the standalone follow-up queue is **still 1** (T9 MP3 narration, parked on user's recording session). The candidate set the user was offered for the next ship, ranked by payoff for the actual 3yo, with **options 2 and 4 now ✅ shipped**:
  ...
  5. **New domain: alphabet / letter recognition** — pivot from numbers to early literacy. Bigger fresh-design space (new themes, new stages, possibly new layouts). **~5–8 hr** for a v1. Higher long-term payoff (broadens curriculum), lower payoff *today* vs. completing the math arc with Number Bond Pop. Probably right move once the math arc closes.
  ...
  **Default order** if the user just says "ship next" without picking (post T-patterns ship): 3 (T9 — gating data lives on `/stats` thanks to T-retention; ship once user signals recordings ready OR ≥3 separate-day return visits land + the user is willing to revisit Tier A scope) → 1 (Number Bond Pop — pending re-evaluation of the age-3 gate; now also weighed against "do we have enough preschool-math? 4 games may be sufficient for v1") → 5 (alphabet — broaden curriculum to early literacy now that math arc has 4 games) → 6 (a11y).

  **Don't pick autonomously without user confirmation.** Each option has a different bet: 1 widens preschool-math; 3 deepens v1 quality; 5 widens to a new domain; 6 widens audience. The user's read on the actual 3yo's engagement is the input the agent doesn't have. **Note**: T-retention + T-patterns shipping this session means future "what next?" calls have two more datapoints — check `/stats` activity panel for return-visit signal across all 4 preschool-math games before recommending T9 OR Number Bond Pop. If Pattern Sequences gets fewer return visits than the cardinality triad, the implication is the kid prefers counting/recognising over pattern-recognising and Number Bond Pop becomes higher-priority. If the inverse, alphabet (option 5) becomes higher-priority because it also widens beyond counting.
