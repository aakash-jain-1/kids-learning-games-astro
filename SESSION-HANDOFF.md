...
- **State (2026-05-20): all 13 vanilla games ported and live (migration
  complete since 2026-05-08), plus 3 new feature-driven games — Counting
  ...
  T2.1 closure, later in the same session), and Number Friends
  (preschool-math numeral recognition, shipped 2026-05-18 — same session
  as More Friends, completes the cardinality triad). Total 16 games** at
...
- **NEXT SESSION CANDIDATES (snapshot 2026-05-20, latest — UPDATED post T-retention ship)**: with the triad shipped + UX-quality work (T-extra + triad-extension) closed AND retention instrumentation (option 4) shipped this same session, the standalone follow-up queue is **down to 1** (T9 MP3 narration, parked on user's recording session). The candidate set the user was offered for the next ship, ranked by payoff for the actual 3yo, with **option 4 now ✅ shipped**:
  ...
  2. **New game: Pattern Sequences** — what the user originally picked at T6 close before pivoting to the wrong-answer-feedback pushback. Cognitive skill complementary to math (red-blue-red-?, big-small-big-?). Different mechanic from the triad → fresh stimulus for the 3yo, lower risk of "another counting game" fatigue. **~3 hr.** Likely the safest 3yo-appropriate option if Number Bond Pop's "to make 5" mechanic ends up being judged too abstract.
  ...
  **Default order** if the user just says "ship next" without picking (post T-retention ship): 2 (Pattern Sequences — fresh mechanic, age-safe, smallest investment) → 1 (Number Bond Pop — pending re-evaluation of the age-3 gate) → 3 (T9 — gating data now lives on `/stats` thanks to T-retention; ship once ≥3 separate-day return visits land) → 5 (alphabet) → 6 (a11y).

  **Don't pick autonomously without user confirmation.** Each option has a different bet: 1+2 widen the v1 scope; 3 deepens v1 quality; 5 widens to a new domain; 6 widens audience. The user's read on the actual 3yo's engagement is the input the agent doesn't have. **Note**: T-retention shipping this session means future "what next?" calls have one more datapoint — check `/stats` activity panel for return-visit signal before recommending T9.
