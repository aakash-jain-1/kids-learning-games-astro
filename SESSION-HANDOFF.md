...
- **NEXT SESSION CANDIDATES (snapshot 2026-05-20, latest — captured at the user's request before they break)**: with the triad shipped + UX-quality work (T-extra + triad-extension) closed, the standalone follow-up queue is **down to 1** (T9 MP3 narration, deferred). The candidate set the user was offered for the next ship, ranked by payoff for the actual 3yo:
  ...
  3. **T9: pre-recorded MP3 narration for the triad** — replaces Web Speech with a warm recorded human voice. Originally deferred because the deferral logic was *"validate v1 retention first; don't sink 3 hr of recording on a v1 that turns out not to land."* If the 3yo has now voluntarily returned to the triad ≥3 times across ≥3 separate days, T9 is unblocked and arguably the highest-quality-of-experience win in the queue. **~2–3 hr** if the user records themselves (research consistently shows familiar parent voices outperform polished VO for retention in the 3–5 age range — Sesame Workshop / Khan Academy Kids design notes), **3–4 hr** with ElevenLabs / similar AI voice (need to design a voice profile + regenerate per phrase + QA), **+5–6 hr** if hiring a Fiverr / Voice123 VO artist. **See the dedicated T9 explainer entry under PROGRESS.md changelog 2026-05-20 (latest, T9 deep-dive)** for the full phrase inventory, registry shape, `narrate()` upgrade plan, service-worker precaching strategy, test plan, and reopen conditions.
