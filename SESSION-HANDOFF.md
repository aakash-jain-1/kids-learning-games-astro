...
- **NEXT SESSION CANDIDATES (snapshot 2026-05-20, latest — captured at the user's request before they break)**: with the triad shipped + UX-quality work (T-extra + triad-extension) closed, the standalone follow-up queue is **down to 1** (T9 MP3 narration, deferred). The candidate set the user was offered for the next ship, ranked by payoff for the actual 3yo:
  ...
  4. **Retention instrumentation** — extend `/stats` (or add a `/parent-stats` page) with a "last-played-at" timestamp per game and a 7-day rolling chart, so the user can *empirically* see whether the 3yo returns voluntarily. **Unblocks T9's deferral logic** — converts the "have I observed return-visits?" gut-check into a number. **~1–2 hr.** Lowest user-facing payoff, but highest leverage on T9's go/no-go decision.
  ...
  **Default order** if the user just says "ship next" without picking: 2 (Pattern Sequences — fresh mechanic, age-safe, smallest investment) → 1 (Number Bond Pop — pending re-evaluation of the age-3 gate) → 4 (instrumentation) → 3 (T9 — only after ≥1 of {observed return-visits, instrumentation data}) → 5 (alphabet) → 6 (a11y).

  **Don't pick autonomously without user confirmation.** Each option has a different bet: 1+2 widen the v1 scope; 3 deepens v1 quality; 4 derisks the T9 decision; 5 widens to a new domain; 6 widens audience. The user's read on the actual 3yo's engagement is the input the agent doesn't have.
