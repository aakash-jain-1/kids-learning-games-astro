...
- **State (2026-05-25): all 13 vanilla games ported and live (migration
  complete since 2026-05-08), plus 5 new feature-driven games — Counting
  Friends (preschool-math addition, shipped 2026-05-15), More Friends
  (preschool-math magnitude comparison, shipped 2026-05-18 — same day as
  T2.1 closure, later in the same session), Number Friends (preschool-math
  numeral recognition, shipped 2026-05-18 — same session as More Friends,
  completes the cardinality triad), Pattern Sequences (preschool-math
  pattern recognition for ages 3–4, shipped 2026-05-20 — same session as
  T-retention, sister to the cardinality triad), and **Letter Friends**
  (preschool-LITERACY uppercase letter recognition for ages 3–4, shipped
  2026-05-25 — first feature-driven game outside the math arc, opens a
  new `preschool-literacy` family on the parent stats dashboard;
  research-grounded design from a 5-paper / 3-curriculum / 3-app
  pre-build pedagogy survey responding to the user's directive *"design
  the game thoroughly and according to a 3-year-old, also check what the
  golden standards are"*). Total 18 games** at
  ...
  *CI hardened 2026-05-18: Playwright is now a hard deploy gate (T2.1
  ...
  and the deploy proceeded only because the gate passed. The Number
  Friends ship that followed was the second feature-driven game to
  ...
  in PROGRESS.md changelog **2026-05-19**. **T6 closed 2026-05-20**
  with `src/pages/stats.astro` + `src/data/stats-registry.ts` +
  `tests/stats.spec.ts` (6 tests, all passing locally with
  77/77 across the full suite). Parent-facing dashboard at
  `/kids-learning-games-astro/stats` renders one card per game
  across 4 family sections; per-card + global Reset (with
  confirm); per-game in-page `alert()` Stats buttons stay in
  place as the additive immediate-feedback view. Registry is
  single-source-of-truth so adding a 17th game = one entry edit
  (the test asserts card count matches `STATS_REGISTRY.length`).
  Queue drops from 2 to 1 — only T9 MP3 narration remains, still
  deferred until v1 retention is validated with the actual 3yo
  user.*
- **Three shared layouts** in production:
  ...
  - `StoryLayout` (7 games — Daily Routines paginated, Honest Woodcutter,
    **Counting Friends** ← shipped 2026-05-15 as the first
    ...
    consumer", **More Friends** ← shipped 2026-05-18 as the
    second feature-driven game; sister/precursor to Counting Friends,
    uses the new `theme='comparison'` key, and **Number Friends** ←
    shipped 2026-05-18 (later same session) as the third feature-driven
    game; completes the cardinality triad — Counting Friends asks
    set→numeral, More Friends asks set vs set, Number Friends asks
    numeral→set ("show me three" — child sees a numeral and finds the
    matching group); uses the new `theme='numberfriends'` key. **The
    StageLayout carve was deferred a third time** with explicit
    re-evaluation conditions now locked into `StoryLayout.astro`'s
    JSDoc — chrome differences still amount to one body-class rename
    (`body.story` → `body.stage`) and nothing else, the hypothetical
    StageLayout would re-import the entire `<head>` and `<body>`
    shell rather than dedupe it, and migrating addition.css +
    comparison.css + numberfriends.css from `body.story[...]` to
    `body.stage[...]` is mechanical churn for zero behaviour change.
    Trigger conditions: a fourth stage-game ships AND a meaningful
    chrome difference emerges; OR the theme union grows to ≥6 keys;
    OR a stage-only style primitive emerges. **The second-consumer
    carve that DID happen at the More Friends ship — `src/lib/preschool-themes.ts` —
    got its third consumer at the Number Friends ship at zero
    changes**, validating that the carve hit the right level of
    abstraction; if any of `PreschoolTheme` / `THEMES` / `THEME_BY_KEY` /
    `numberWord` / `cap` / `nounFor` had needed reshaping for the
    third game, the carve would have been premature; none did).
- **Just shipped (this session, 2026-05-25, latest — T-letters)**: **`feat(games):` Letter Friends — first preschool-LITERACY game; uppercase letter recognition for ages 3–4.** Picked from option 5 in the 2026-05-20 candidate set after the user said "Continue" twice — the math arc had four games (cardinality triad + Pattern Sequences) and the obvious next axis was *broaden curriculum to a new domain*. The user explicitly directed *"design the game thoroughly and according to a 3-year-old, also check what the golden standards are"*, which redirected the build from "ship using the Number Friends template" to "research the pedagogy first, then ship with research-grounded defaults baked in." **The pre-build pedagogy survey** covered 5 peer-reviewed papers (Piasta et al. 2010 RCT, Frontiers 2026 picture-mnemonics RCT, Springer 2025 uppercase-facilitation, Davidson 1935 mirror-letter, Phonics.org reversals literature), 3 curriculum guides (Jolly Phonics UK, Letterland UK, Montessori), and 3 shipping preschool apps (Khan Academy Kids, Endless Alphabet, ABCmouse). Findings that drove design: uppercase first (visually simpler, more distinct, consensus across all sources except Montessori), combined name+sound narration (Piasta 2010 RCT showed combined > sound-only or name-only for preschoolers), SATPIN-inspired progression NOT alphabetical (alphabetical makes kids revert to letter NAMES instead of attending to shapes/sounds), picture mnemonics embedded (Dual Coding Theory + Frontiers 2026 RCT), confusable-pair denylist (B/D, P/Q, M/W, M/N, E/F, I/L, U/V, V/W, O/Q — never co-occur as target+distractor; mirror-letter confusion is developmental at age 3, not a red flag, but no need to *deliberately* induce it), no scoring or timers (Khan Kids / Endless Alphabet / ABCmouse all explicitly avoid these for ages 2–5). **Design fork resolved with the user**: research recommended a 12-letter pool to avoid overload, but the user explicitly chose **all 26 letters**. Adapted by tier-weighting each 8-round session — tier 1 (S A T P I N, Jolly Phonics Set 1) gets 2 round slots; tier 2 (M D G O C K) gets 2 slots; tier 3 (E U R H B F L) gets 2 slots; tier 4 (J V W X Y Z Q) gets 2 slots. Across many sessions every letter appears as a target, weighted by tier. **The shipped data layer** `src/data/letterfriends.ts` (~330 LoC) reuses `ALL_CARDS` from `src/data/alphabets.ts` for letter→word→emoji mnemonics (A-Apple, …Z-Zebra, with the existing Q→Crown substitution preserved verbatim — zero new content). Exposes `LetterId`, `LetterMeta`, `LetterRound`, `ALL_LETTERS` (26 caps), `LETTER_META_BY_ID`, `TIER_1_TARGETS`/.../`TIER_4_TARGETS`, `CONFUSABLE_PAIRS` + `confusableWith()` helper, `generateSession(rand)` (8 rounds, 2 per tier, distractor pool = A-Z minus target minus its confusables), `buildNarration(round)` (combined "A. A is for Apple." narration line), `STATS_KEY = 'letter_friends_stats_v1'` + `loadLetterFriendsStats` / `saveLetterFriendsStats` (same `{ sessions, rounds, correctFirstTry, lastPlayed }` shape as the cardinality triad — deliberate so the same stats-registry factory takes the new game with one new `family` arg). **The shipped page** `src/pages/games/letter-friends-game.astro` (~480 LoC) uses StoryLayout with new theme key `'letterfriends'`, renders one big uppercase target letter at the top with its word + emoji mnemonic ("A is for Apple"), three letter tiles below with consistent uppercase typeface, errorless wrong-tap flow (250 ms `lf-tile--wrong` shake → "Hmm! Let's look together." → narrate the wrong tile + redirect to target → "Look! This is A. A is for Apple." → reveal correct tile with pulsing pink ring → enable Next), same SSR-derived round 0 + readSSRRound kickoff pattern as the cardinality triad. **The shipped CSS** `src/styles/letterfriends.css` (~580 LoC) under `body.story[data-theme='letterfriends']`, mirrors the triad CSS shape with pink accent (#ef476f) instead of green to differentiate the literacy family from the math family at a glance. Age-safe `lfShake` keyframe (NO color shift — same constraint as `nfShake`/`mfShake`/`cfShake`/`psShake`); reduced-motion fallback collapses every animation; dark-mode tokens included from day one; mobile responsive. **StoryLayout theme union widened to include `'letterfriends'` + pre-dark FOUC dark-bg token added; 5th-time StageLayout deferral documented** with the same revised trigger conditions from the Pattern Sequences ship — chrome differences still amount to one body-class rename, the carve must deduplicate something REAL to pay for the migration churn. **Wired into GameNav (one new "Letter" link), home page (one new card "🔤 Letter Friends"), `stats-registry.ts` (NEW preschool-literacy family added — StatsFamily widened from 4→5 keys, `preschoolMathEntry` factory generalized to `preschoolStatsEntry` with a `family` parameter so the same factory wires both math and literacy games at zero shape-divergence; FAMILY_LABELS / FAMILY_COLORS / FAMILY_SIZES / zeroPerFamily / FAMILY_ORDER all updated; preschool-math relabeled to "Preschool math (cardinality + pattern)" to honestly describe what's in that bucket now)**, `tests/stats.spec.ts` (`EXPECTED_GAME_IDS` +1, family-section count 4→5, activity-grid dot count 28→35, activity-panel hydration test seeds letter-friends), and `tests/letterfriends.spec.ts` (NEW, 7 cases mirroring numberfriends.spec.ts plus a stats-page family-section test specific to literacy). **Build deltas:** 9 files changed (4 new + 5 modified), ~1860 LoC net new. `npm run check` 0/0/0 across **59 Astro files** (was 58). `npm run build` 21 pages built clean (was 20). Service worker precaches **101 entries** (was 97 — +4 for the new page HTML + page JS chunk + new `letterfriends.css` + the existing `alphabets.ts` data chunk gains a second consumer). Verified dist artifact: `letter-friends-game.html` ships `data-theme="letterfriends"`, the SSR'd target letter card with picture mnemonic, three SSR'd letter tiles, the home card link, and the `preschool-literacy` family entry on `/stats`. **Local Playwright still blocked by Zscaler 403 + sandbox-only Apple Silicon detection failure** (Node 24 + macOS sandbox returns empty `os.cpus()`); relying on the deploy.yml hard gate as before. **Standalone follow-up queue still 1** (T9 still parked on the user's recording session); 5 preschool games now shipped (4 math + 1 literacy). Full ADR under PROGRESS.md changelog **2026-05-25 (latest)**.

- **Earlier this session (2026-05-20 — T-patterns)**: **`feat(games):` Pattern Sequences — fourth preschool-math game; sequential pattern recognition for ages 3–4.** Picked from the post-T-retention "what next?" candidate set after the user explicitly chose to keep going. Of the remaining agent-actionable options (T9 was parked on the user's recording session, Number Bond Pop pending age-3 fitness re-evaluation, alphabet/a11y considered lower-priority for the actual 3yo user), Pattern Sequences was the only candidate that gave the 3yo a *new mechanic* to play AND generated fresh signal on the activity panel that just shipped (T-retention) — the next return visit lights a new family-coloured dot on the 7-day grid, validating both the new game and the panel in one go. **The shipped data layer** `src/data/patterns.ts` (~280 LoC) exposes `PatternColor` (4 named colors), `PatternKind` (`'AB' | 'AAB' | 'ABB' | 'ABC'` — four difficulty tiers covering the entire age-3-to-4 pattern-recognition curriculum), `PatternRound` (immutable round shape with `kind`, `sequence`, `correctColor`, `options`, `correctIndex`, `theme`), `generateSession(rand)` (8 rounds, 2 of each tier, post-shuffle), `buildNarration(round)` (`{ intro, correct, rerun, reveal, colorWord(c) }`), `STATS_KEY = 'pattern_sequences_stats_v1'`, `loadPatternStats` / `savePatternStats` (same `{ sessions, rounds, correctFirstTry, lastPlayed }` shape as the cardinality triad — deliberate so the existing `preschoolMathEntry` factory in `stats-registry.ts` takes the new game with zero shape changes). **Design fork resolved before build: pattern primitive = colored circles** (universal across all 4 themes). Three options were considered; colored circles won on pre-attentive discriminability (color is <200 ms vs ~400 ms for emoji), working-memory load (single attribute vs 2 for color+shape), curricular alignment (reinforces existing `/games/colors-game`), pedagogical precedent (Piaget seriation tasks, Montessori bead-stringing, EYFS pattern work — all use colored primitives for ages 3–4; shapes get introduced age 5+), and audio rhythm ("red... blue... red..." — short say-along-able words). The 4 themes drive bg + caption + ambient flavor only; pattern primitive is theme-INDEPENDENT — same architecture as `numberfriends.css` (numerals are theme-independent there). **The shipped page** `src/pages/games/pattern-sequences-game.astro` (~430 LoC) uses StoryLayout with new theme key `'patterns'`, renders 5 colored circles + a "?" slot at position 5 with 3 colored option buttons below, errorless wrong-tap flow (250 ms `ps-opt--wrong` shake → "Hmm! Let's look at the pattern" → walk the visible sequence highlighting each item while speaking its color → reveal the correct option AND fill the slot with the correct color → narrate answer → enable Next), same SSR-derived round 0 + readSSRRound kickoff pattern as the triad. **The shipped CSS** `src/styles/patterns.css` (~440 LoC) under `body.story[data-theme='patterns']`, mirrors the triad CSS shape (per-theme bg tokens, fly-in / pulse / bounce / pulse-ring / shake keyframes, age-safe `psShake` with NO color shift — same constraint as `nfShake`/`mfShake`/`cfShake`), reduced-motion fallback collapses every animation, dark-mode tokens included from day one, mobile responsive (44 px circles below 480 px). **StoryLayout theme union widened to include `'patterns'` + pre-dark FOUC dark-bg token added; 4th-time StageLayout deferral documented** with revised trigger conditions: the "Nth game count" alone is no longer sufficient — through 4 games we've proven the body-class scope IS the deduplication mechanism, so the carve must deduplicate something REAL (different head meta, different header, different settings panel) to pay for the migration churn. **Wired into GameNav (one new "Pattern" link), home page (one new card "🎨 Pattern Sequences"), `stats-registry.ts` (one factory call), `EXPECTED_GAME_IDS` in `tests/stats.spec.ts` (+1 entry)** — all the lockstep edits any new preschool-math game needs. **Tests in `tests/patterns.spec.ts`** (~270 LoC, 5 cases) mirror `numberfriends.spec.ts`: SSR shape (5 `.ps-circle` + 1 `#psSlot`, 3 distinct option colors, recognised data-kind ∈ `{AB,AAB,ABB,ABC}`), Next-disabled-without-answer, tap-any-option-eventually-enables-Next, **deterministic correct-tap path** (reconstructs the correct color from `data-kind` + the visible sequence by applying the cycle at position VISIBLE_LENGTH — more robust than hard-coding the seeded round's answer), wrong-tap path (250 ms `ps-opt--wrong` shake assertion within 1 s, then `ps-opt--reveal` on the correct option after the slow chain, `correctFirstTry === 0`). **Build deltas:** 8 files changed (4 new + 4 modified), ~1420 LoC net new. `npm run check` 0/0/0 across **57 Astro files** (was 55). `npm run build` 20 pages built in 8.42 s. Service worker precaches **97 entries** (was 93 — 4 new entries for the new page HTML + page JS chunk + new `patterns.css` + the deduped shared `_astro/preschool-themes.<hash>.js` chunk now has its 4th consumer). Verified dist artifacts: SSR'd HTML has `data-theme="patterns"`, 5 `ps-circle` instances, 3 `ps-opt` buttons, home card link to `pattern-sequences-game`, stats registry includes the new entry. **Local Playwright still blocked by Zscaler 403 + sandbox-only Apple Silicon detection failure** (Node 24 + macOS sandbox returns empty `os.cpus()` → Playwright falls back to the `mac-x64` browser path, which doesn't have an arm64 binary; documented in `playwright.config.ts`). Relying on the deploy.yml hard gate as before. **Standalone follow-up queue still 1** (T9 still parked on the user's recording session); 4 preschool-math games now shipped (cardinality triad + Pattern Sequences). Full ADR under PROGRESS.md changelog **2026-05-20 (latest)**.

- **Earlier this session (2026-05-20 — T-retention)**: **`feat(stats):` retention instrumentation — sitewide play-history + 7-day activity panel + relative-time "last played".** Picked from the 5-item queue documented in the next bullet, after T9 (the 6th candidate) was parked waiting on the user's recording session. Of the 5 agent-actionable options, retention instrumentation was both the smallest (~1.5 hr) and the highest-leverage one — shipping it converts the *"is the 3yo coming back?"* question from gut-check to a chart, so when the T9 voiceover recordings are ready, the call to ship vs. wait is data-backed instead of vibes-based. **The shipped lib** `src/lib/retention.ts` (200 LoC) owns a single sitewide LocalStorage key `kids_play_history_v1` shaped `Record<YYYY-MM-DD, string[]>` (date → array of gameIds played that day, deduped). Public API: `recordPlay(gameId)`, `getPlayHistory()`, `clearPlayHistory()`, `fmtRelativeDate(iso)`, `lastNDays(n)`, `weekdayShort(iso)` — all SSR-safe, all storage-failure noops. Rolling 30-day window so storage stays bounded. **Local-time YYYY-MM-DD**, not UTC — retention from the parent's perspective tracks "did they play today" relative to the wall clock; UTC midnight feels wrong if the kid plays at 11pm Monday-local-time but UTC says Tuesday. **Why a sitewide key (not per-schema bumps).** Each per-game schema already tracks `lastPlayed` (ISO date string), enough for "last played: today" on the per-card row. Cross-game activity (the chart question) is one level up — fundamentally a sitewide datapoint by definition. The alternative — adding `playHistory: readonly string[]` to 5 schemas — meant 5 schema bumps with backward-compat defaults and 5+ writer call sites coordinating dedup. The single sitewide key is dramatically simpler: one new lib, one write call (`recordPlay(gameId)`), one read for the chart, per-card "last played" stays unchanged. **Wired into 5 writer sites** so a 17th game inherits retention recording for free: per-round + per-session writers in the 3 preschool-math pages, `saveQuizState` in `src/lib/quiz.ts` (covers all 13 mountQuiz games via the shared controller), `saveLearned` in `src/lib/progress.ts` (covers card-set tile-taps so we record activity even when the kid doesn't open the quiz). **Stats registry upgrades:** `fmtLastPlayed` is now an alias for `fmtRelativeDate` so all 16 cards show "today" / "yesterday" / "3 days ago" / "last week" / "2 weeks ago" / ISO-fallback / "never" instead of raw ISO. New `getActivityByFamily(daysBack = 7)` projects the sitewide history through the gameId → family mapping, returns `readonly DailyActivity[]` (oldest first, today last) where each entry has `{ date, perFamily, total }`. New `FAMILY_COLORS` (hex per family — green/blue/amber/purple) and `FAMILY_SIZES` (denominator). **`src/pages/stats.astro` — new "📅 7-day activity" panel** at the top of `<main>`, above the family sections. Single row of 7 day cells (oldest left, today right with highlighted bg + outline). Each cell stacks **4 family-coloured dots**: an active dot (`.is-active`) means at least one game in that family was played that day. 4-item legend below. Mobile breakpoint at 480px tightens spacing and hides date row. SSR ships all 28 dots inactive (server has no localStorage); hydration replays `getActivityByFamily(7)` and toggles `.is-active` per dot — pure class toggle, no DOM rebuild, no layout shift. **"Reset everything" now also calls `clearPlayHistory()`. Per-card "Reset" does NOT clear the history** — rationale: erasing the activity calendar when a parent resets one game's stats would be unhelpful. Locked into a dedicated test so the design rationale can't silently drift. **Tests in `tests/stats.spec.ts`:** 5 new test cases (SSR shape lock, hydration with seeded history, "Reset everything" wipes the key, per-card reset preserves the key, end-to-end Counting Friends round → today's date in `kids_play_history_v1` + per-card row reads "today" + first dot lit) + 1 existing test updated to seed JS-computed today/yesterday and assert the new "today"/"yesterday" relative format. **Build deltas:** 9 files changed, +600 LoC net (mostly the new lib + the new tests + the panel CSS). `npm run check` 0/0/0 across all 55 Astro files. `npm run build` 19 pages in ~9s. Service worker still precaches 93 entries (was 91 — 2 more for the new `_astro/retention.<hash>.js` shared chunk and re-hashed stats.html / stats.<hash>.js). `dist/stats.html` ships 28 dot elements + 7 day cells + 4 legend items + 11 `data-family="preschool-math"` matches (3 cards + 1 section + 7 dots = 11). **Local Playwright still blocked by Zscaler** — relying on the deploy.yml hard gate. **Standalone follow-up queue still 1** (T9 still parked waiting on the user's recording). Full ADR under PROGRESS.md changelog **2026-05-20 (latest)**.

- **Earlier this session (2026-05-20 — T-extra triad-extension)**: **`feat(triad):` extend the age-safe wrong-tap shake to Counting Friends + More Friends + Number Friends.** User-driven follow-up filed minutes after the T-extra ship landed — they noticed the new shake pattern only fired on `mountQuiz` games (Woodcutter, Daily Routines quiz, the 11 grid + card-machine games) and not on the three preschool-math games their actual 3yo plays. The original T-extra rationale for skipping the triad — *"the shake would compete with the verbal 'Let's count them together' cue"* — doesn't survive scrutiny: the shake is 250 ms and the narration is 1–2 s (different timescales), and the shake is purely visual while the narration is purely audio (different channels). Without the shake, the kid taps and visually NOTHING happens for ~300–500 ms until the guided count starts highlighting items, which is worse than the new mountQuiz pattern from a "did my tap register?" UX perspective. The age-safe shake design is exactly that — age-safe — so the only reason to keep it out was "different visual languages per game family," which has weak weight when consistency benefits the actual user. **The fix is purely additive** — each triad game now adds `<prefix>--wrong` to the tapped element immediately on the wrong branch, with a matching `<prefix>Shake` keyframe (byte-for-byte identical 4-frame translateX, namespaced per the existing `<prefix>Bounce` / `<prefix>PulseRing` convention). NO color shift, NO desaturation, NO negative tone — purely kinesthetic confirmation that the tap registered. The existing errorless-rerun flow (cancel speech → caption "Let's count them together" → guided count → reveal correct with pulse ring) stays byte-for-byte identical. Per-round cleanup extended in More Friends + Number Friends `renderRound` (Counting Friends auto-cleans because it rewrites `optionsEl.innerHTML` per round). Tests extended on each of `addition.spec.ts` / `comparison.spec.ts` / `numberfriends.spec.ts` to assert the `--wrong` class lands on the tapped element within 1 s of click, BEFORE the existing reveal-chain assertion. **Build verified:** each triad game's per-game CSS chunk contains both the new selector and the new `<prefix>Shake` keyframe; each game's per-page JS chunk contains the `'<prefix>-wrong'` string literal driving the `classList.add` call. **Build deltas:** 9 files changed, +87 LoC net. CSS bundles +110–130 bytes each (3 rule + 1 keyframe). Precache 91 entries (was 90 — three CSS chunks re-hashed). `npm run check` 0/0/0 across 54 Astro files. Local Playwright still blocked by Zscaler — relying on the deploy.yml hard gate. **Standalone follow-up queue still 1** — only T9 MP3 narration remains. Full ADR under PROGRESS.md changelog **2026-05-20 (latest)**.

- **NEXT SESSION CANDIDATES (snapshot 2026-05-25, latest — UPDATED post T-letters ship)**: with the cardinality triad shipped + Pattern Sequences (option 2) shipped + retention instrumentation (option 4) shipped + Letter Friends (option 5) shipped — the standalone follow-up queue is **still 1** (T9 MP3 narration, parked on user's recording session). 5 preschool games shipped total (4 math + 1 literacy). The candidate set the user was offered for the next ship, ranked by payoff for the actual 3yo, with **options 2, 4, and 5 now ✅ shipped**:

  1. **New game: Number Bond Pop** ("How many more to make 5?") — earmarked since the Counting Friends ship as the natural fourth preschool-math game. Completes the early-math arc: **compare → count → recognise → decompose**. Reuses `src/lib/preschool-themes.ts` (existing 4 themes), `StoryLayout`, the new shake pattern, errorless flow. **~3–4 hr** including tests. **Highest learning-value-per-hour for a 3yo right now.** **Note from the More Friends → Number Friends queue**: a previous session's notes flagged "missing-addend partitioning is age 4+ which doesn't match the 3yo target user." Re-evaluate that gate before committing — the design space for "to make 5" can be made age-3-safe (e.g. use ten-frames-style filled-vs-empty dots so the kid SEES the gap rather than abstractly partitioning), but if the gate holds, prefer option 2 instead.

  2. **✅ Pattern Sequences — SHIPPED 2026-05-20** as `feat(games): Pattern Sequences`. Fourth preschool-math game; sequential pattern recognition for ages 3–4 across 4 difficulty tiers (AB / AAB / ABB / ABC). Pattern primitive = colored circles (universal across all 4 themes; pre-attentively most discriminable for age 3, reinforces the existing `/games/colors-game`, canonical Piaget seriation primitive). Reuses StoryLayout + preschool-themes lib (4th consumer, validating the second-consumer carve once more). Same `{ sessions, rounds, correctFirstTry, lastPlayed }` schema as the triad → registered in `stats-registry.ts` via the existing `preschoolMathEntry` factory at zero shape changes. See PROGRESS.md changelog **2026-05-20 (latest)** for the design-fork ADR that locked colored circles as the pattern primitive.

  3. **T9: pre-recorded MP3 narration for the triad** — replaces Web Speech with a warm recorded human voice. Originally deferred because the deferral logic was *"validate v1 retention first; don't sink 3 hr of recording on a v1 that turns out not to land."* If the 3yo has now voluntarily returned to the triad ≥3 times across ≥3 separate days, T9 is unblocked and arguably the highest-quality-of-experience win in the queue. **Total work estimate ~75 min for Tier A** (15 min setup + 30 min recording + 30 min post-processing) **+ 30–45 min agent integration** when the user has the MP3s ready. **🔒 Tier A scope LOCKED IN 2026-05-20**: 13 phrases — numbers `one` through `ten` + `Let's count them together!` + `Hmm! Let's count them together.` + `Look!` — covering ~80% of the parent-voice exposure the 3yo will perceive in a typical play session. Tier B (theme intros, "Find {N}!" prompts, session-complete celebrations) is explicitly deferred until Tier A is shipped and observed; Tier B.5 (full (a, b, sum) celebration coverage for Counting Friends) is recommended to be skipped entirely (Web Speech is adequate for once-per-round celebration phrases). **🔒 Recording approach LOCKED IN by the user 2026-05-20**: **Option A — user records themselves.** **The full step-by-step recording walkthrough lives in `docs/T9-RECORDING-GUIDE.md` (room setup, mic positioning, multi-take strategy, delivery notes per phrase, Audacity post-processing including loudness normalisation to −16 LUFS, ffmpeg one-liner for batch normalisation, file-naming convention, drop-into-`src/assets/narration/shared/`, hand-off-to-agent instructions).** **The literal phrase inventory lives in `docs/T9-PHRASE-SCRIPT.md` (the exact strings to read aloud, organised by tier with delivery notes per phrase — the 3yo's primary attachment voice argument means the user reads each phrase as if to their 3yo right now, not as a VO).** **When the recordings are ready, the user opens a fresh agent session and says "T9 recordings are ready. Wire them in."** — the integration agent will read both docs, create `src/lib/narration-assets.ts` (registry of phrase keys → imported MP3 URLs), upgrade `src/lib/speech.ts → narrate()` to look up MP3s in the registry and play via `<audio>` element with Web Speech fallback, configure `@vite-pwa/astro` precaching for the new MP3s (or hybrid runtime-cache strategy depending on precache budget at the time), add Playwright tests verifying the registry is consulted before Web Speech for canonical phrases, build + verify + commit + push. The 3yo target user is *the* specific user (not a generic kid), so the parent-attachment-voice argument from Sesame Workshop + Khan Academy Kids research carries decisive weight, and the variable-home-studio-quality risk is a smaller error than the not-the-parent's-voice risk. Option C (ElevenLabs voice cloning at $22/mo Creator tier) is locked in as the **v2 upgrade path** if the games ever scale beyond the single 3yo or the parent wants to extend the phrase inventory later without re-recording. **See the dedicated T9 explainer entry under PROGRESS.md changelog 2026-05-20 (latest, T9 deep-dive)** for the full phrase inventory (~80 unique phrases after dedup), registry shape, `narrate()` upgrade plan, service-worker precaching strategy (eager vs. hybrid vs. lazy — recommended hybrid keeps precache count under 110 with ~180 KB install bandwidth bump), test plan, reopen conditions, **industry recording standards** (MP3 64 kbps mono, 44.1 kHz, −16 LUFS, ≤50 ms head/tail trim), **AI VO market overview** (ElevenLabs / Play.ht / Microsoft Azure Neural / KidsStoryteller.ai pricing + child-voice availability — surveyed 2026-05-20), **VO marketplace landscape** (Voices.com / Voice123 / Bodalgo / Fiverr / Backstage prices + turnaround), and the **recording-session checklist** (quiet room, phone-or-USB mic, one-sitting capture, slightly slow + warm + slightly higher pitch, Audacity / `ffmpeg loudnorm` for normalisation, file-naming convention, drop into `src/assets/narration/<game>/`).

  4. **✅ Retention instrumentation — SHIPPED 2026-05-20** as `feat(stats): retention instrumentation`. `/stats` now renders a 7-day activity panel at the top showing per-family play activity each day; every per-card "Last played" cell shows relative time ("today" / "yesterday" / "3 days ago" / "last week" / etc.) instead of raw ISO; sitewide `kids_play_history_v1` key tracks daily play across all 16 games via 5 wired-up writer sites. Now you can empirically see whether the 3yo returns voluntarily — converts the T9 "have I observed return-visits?" gut-check into data on `/stats`. See PROGRESS.md changelog **2026-05-20 (latest)**. **Use this data when deciding T9 ship vs. wait timing.**

  5. **✅ Letter Friends — SHIPPED 2026-05-25** as `feat(games): Letter Friends`. First preschool-LITERACY game; uppercase letter recognition for ages 3–4. SATPIN-tiered progression covers all 26 letters across sessions (tier-weighted 8-round sessions, 2 rounds per tier). Combined name+sound narration with picture mnemonic ("A is for Apple"). Confusable-pair denylist (B/D, P/Q, M/W, M/N, E/F, I/L, U/V, V/W, O/Q). Reuses `src/data/alphabets.ts` (zero new content), StoryLayout (5th preschool consumer; StageLayout deferred 5th time), the same `{ sessions, rounds, correctFirstTry, lastPlayed }` schema. Opens NEW `preschool-literacy` family on the parent stats dashboard — `StatsFamily` widened 4→5 keys, `preschoolMathEntry` factory generalized to `preschoolStatsEntry` with a `family` parameter. See PROGRESS.md changelog **2026-05-25 (latest)** for the design ADR (5-paper / 3-curriculum / 3-app pre-build pedagogy survey, all-26-letters fork resolved by tier-weighting).

  **Possible v2 / sister games (Letter Friends follow-ups)**:
  - **Lowercase letters** — once v1 retention is validated (3yo voluntarily returns ≥3× on `/stats`), add 26 lowercase rounds. Same UI grammar; uppercase + lowercase as distractors becomes the advanced mode.
  - **Letter sounds as primary target** ("Find the letter that says /a/!") — phonemic-awareness sister game, complementary to v1's name-recognition pedagogy.
  - **Letter blending / Word Friends** (CVC words like "sat", "tap") — Jolly Phonics Set 1 unlocks blending immediately after S, A, T are known. Future sister game.

  6. **Accessibility audit** — keyboard nav, focus states, contrast ratios, screen-reader labels. Useful, but the 3yo doesn't use a keyboard or screen reader. Defer unless the user wants the games to also work for older audiences, screen-reader users, or motor-impaired users.

  **Default order** if the user just says "ship next" without picking (post T-letters ship): 3 (T9 — gating data lives on `/stats` thanks to T-retention; ship once user signals recordings ready OR ≥3 separate-day return visits land + the user is willing to revisit Tier A scope) → **Letter Friends v2 lowercase** (only after Letter Friends v1 retention is validated on `/stats`) → 1 (Number Bond Pop — pending re-evaluation of the age-3 gate; weighed against "do we have enough preschool-math? 4 games may be sufficient for v1 of the math arc") → 6 (a11y).

  **Don't pick autonomously without user confirmation.** Each option has a different bet: 1 widens preschool-math; 3 deepens v1 quality; Letter Friends v2 deepens preschool-literacy (lowercase / sounds / blending); 6 widens audience. The user's read on the actual 3yo's engagement is the input the agent doesn't have. **Note**: T-retention + T-patterns + T-letters shipping this session arc means future "what next?" calls have three datapoints to weigh — check `/stats` activity panel for return-visit signal across the 4 math games AND the new Letter Friends literacy game before recommending T9, Letter Friends v2, or Number Bond Pop. If Letter Friends gets meaningfully more return visits than the math games, that's a clear signal to deepen literacy (v2 lowercase / sounds / Word Friends) before adding a 5th math game.

- **Earlier this session (2026-05-20, later — T-extra)**: **`feat(quiz):` age-safe wrong-answer feedback on all 13 `mountQuiz` games.** User-driven follow-up filed at the close of T6 — they noticed the asymmetry that "correct option celebrates, wrong option is silent" and asked what the standard is. Diagnosing the actual code confirmed it: every `mountQuiz` game (2 story + 7 grid + 4 card-machine) ran the silent two-statement `if (i === q.ans) correct++; idx++; renderQuestion();` body, so a wrong tap got *zero* feedback. The first proposal (early-elementary shake + desaturate + reveal-correct + soft downward tone) got pushed back on by the user — *"is it okay to do this for a 3 year old? You said it is not okay right?"* — which forced an honest age-split: red flash + buzzer is harmful for ages 3–4 (Skinner / Touchette errorless-learning), even mild desaturate + downward tone is borderline-not-okay for a 3yo, only the *kinesthetic* shake (no color, no negative tone) survives the safety bar across all ages. Downgraded to an **age-safe variant**: shake on the tapped wrong button (`quiz-opt--wrong`, 250 ms 4-frame translateX, no color shift) + green-ring pulse on the actual correct button (`quiz-opt--reveal`, 600 ms scale + box-shadow, #22c55e outline) + 700 ms advance gate; correct taps get a 450 ms scale-pop on the tapped button (`quiz-opt--correct`) + 450 ms advance gate. **The preschool-math triad (Counting Friends, More Friends, Number Friends) is intentionally NOT touched** — it doesn't use `mountQuiz`, it has its own page-local errorless flow (guided-count rerun + correct-answer reveal) which is the research-grounded standard for ages 3–4 and would actively regress if any cue from the older-audience pattern were grafted on. The new feedback rules live in `src/styles/global.css` (the existing global `prefers-reduced-motion` block at the bottom already nullifies all three keyframes to 0.01 ms — no per-rule override needed); they're scoped under `.quiz-opt` which only exists inside `mountQuiz`-rendered DOM, so zero leakage to any other component. All three layouts (CardMachineLayout, GridLayout, StoryLayout) already import `global.css` so the rules ship to every quiz game with zero per-layout CSS work — verified at build time: the shared CSS chunk emitted under one entry's name (`dist/_astro/dinosaurs-game.DgDE2SZE.css`, Vite chunking artefact) is referenced by every page; the rules are emitted into it correctly. **`src/lib/quiz.ts → onAnswer`** upgraded to add classList writes + button-disable + delayed-render, with a multi-paragraph JSDoc above it documenting the age-safe rationale so future contributors don't re-introduce shame-coded feedback by accident. **`tests/story.spec.ts`** gained a new `describe('wrong-answer feedback (mountQuiz)')` block with 3 cases pinned to Woodcutter Q1 (deterministic `ans: 1` lets us deterministically map `data-i="0"` to a wrong tap and `data-i="1"` to a correct tap without injecting test fixtures): wrong tap shape (tapped gets `--wrong`, correct gets `--reveal`, all four buttons disabled, Q2 renders within 2 s, all classes gone after re-render); correct tap shape (tapped gets `--correct`, no `--wrong` or `--reveal` anywhere, Q2 renders within 450 ms gate); double-tap re-entrancy (second tap during the feedback window via `dispatchEvent('click')` doesn't fire `onAnswer` twice — state stays single `--wrong` + single `--reveal`). Smoke-testing on woodcutter (one consumer of `mountQuiz`) is sufficient because the feedback CSS is global and the controller is shared across all 13 quiz games — no per-layout behavioural divergence. **`tests/helpers.ts → answerQuizUntilResult`** updated to poll for the new feedback gate to settle between clicks instead of clicking through synchronously; existing 7 quiz tests (3 routines + 4 woodcutter) still pass without modification because their assertions are about end-state. **Build deltas:** quiz.ts +30/−2, global.css +52, story spec +90, helpers +20/−9 (net **+181 LoC**); shared CSS chunk +~410 bytes gzip; shared `quiz.<hash>.js` bundle +~170 bytes gzip; precache **90** entries (unchanged — no new files emitted, only existing chunks re-hashed). `npm run check` 0/0/0 across 53 Astro files. **Local Playwright is blocked by Zscaler** on this dev box (documented in `playwright.config.ts`); CI verification deferred to post-push, build sanity confirmed via shared CSS chunk grep. **Standalone follow-up queue stays at 1** — T9 MP3 narration still deferred until v1 retention validation. Full ADR under PROGRESS.md changelog **2026-05-20 (later)**.

- **Earlier this session (2026-05-20)**: **`feat(stats):` close T6 with a parent-facing `/stats` dashboard backed by a single-source-of-truth registry.** Picked from the 2-item follow-up queue T6/T9 — T9 still explicitly deferred until v1 retention is validated with the actual 3yo. T6 won the slot because it has clear parent-facing value (parents currently have to play each game and tap its Stats button to see how the kid's doing per-game, which is friction with 16 games shipped) and is defer-safe (Playwright tests lock the *storage shape* of `<game>_stats_v1` keys, not the per-game `alert()` text — refactoring the alert later remains safe because it was never the contract). **The shipped page** lives at `/stats`, follows the `index.astro`/`404.astro` self-contained-shell pattern (no shared layout — structurally different from any game shell), uses the shared `<GameNav />` + `<SettingsModal />` + `<BuildInfo />`, and renders one card per game across **four family sections** (preschool-math / story / card-set / card-pure) with family-tinted left borders so the section grouping reads at a glance. Each card has a click-through to the game, 3-4 metric rows that mirror what the in-page `alert()` shows, and a 🗑️ Reset button (with `window.confirm()` that names the game). The header strip has 🏠 Home + ⚙️ Settings + a danger-tinted 🗑️ Reset everything button. **The registry** (`src/data/stats-registry.ts`, 318 LoC) is the contract: `StatsRegistryEntry` has `read(): readonly MetricRow[]` (returns ordered display rows from the relevant LocalStorage keys via the existing `loadAdditionStats` / `loadComparisonStats` / `loadNumberFriendsStats` / `loadQuizState` / `loadLearned` shared helpers), `clear(): readonly string[]` (wipes the relevant keys and returns them — handy for tests), `hasData(): boolean` (drives the "No plays yet" badge + `is-empty` opacity dim). `STATS_REGISTRY` is consumed by *both* `stats.astro` and `tests/stats.spec.ts`, so adding a 17th game means one entry edit (and a one-line append to `EXPECTED_GAME_IDS` in the test). **No new shared lib in `src/lib/`** — per rule #3 ("refactor trigger = second consumer"), the registry is the FIRST consumer of a hypothetical "unified game-stats reader" abstraction; nothing was carved out. If a second cross-game tool ever needs the same registry shape (e.g. a future "export to CSV" feature), then `src/lib/stats.ts` extraction is on the table — but with one consumer it would be premature. **SSR strategy:** every card renders the *zero-state* values that `entry.read()` returns server-side (where `localStorage` is undefined → loaders return their `ZERO_STATS` shape). The hydrating script then re-runs `entry.read()` against real LocalStorage and patches the value spans + `is-empty` class + "No plays yet" badge. For first-time visitors there is no flicker (zero is the correct value); returning parents see a brief flash of zero before the real numbers appear, which matches how every existing in-page Stats `alert()` already works. **Per-game alerts stay in place — additive, not a replacement.** The /stats page is the *cross-game aggregation* view; the per-game alert is the *immediate feedback while playing this specific game* view. Both have value. **Files:** *new* `src/data/stats-registry.ts` (~318 LoC), `src/pages/stats.astro` (~343 LoC), `tests/stats.spec.ts` (~312 LoC, 6 tests); *changed* `src/components/GameNav.astro` (+1 link "📊 Stats" with subtle opacity + `game-nav-stats` class), `src/pages/index.astro` (+1 small "📊 View parent stats →" link below the home grid, deliberately not a `.home-card` because /stats isn't a game). **Verifications:** `npm run check` 0/0/0 across **53 Astro files** (+1 from the 52 baseline — `stats.astro`); `npm run build` emits **19 pages** (+1) with `dist/stats.html` at 36 KB containing all 16 `data-game-id` attrs + 4 family section blocks + zero-state SSR values (`never` × 16, `0 / 0` × 3 first-try rows, etc.); precache **90 entries** (+17 — the page + chunks for the new registry import + the per-game data file imports the registry pulls in for `ALL_CARDS.length` denominators; well under the 100-entry safety budget). **Local Playwright suite: 77/77 passing** including all 6 new T6 tests + the 71 pre-existing tests with no regressions. **Standalone follow-up queue drops to 1** — only T9 MP3 narration remains, still deferred. Full ADR under PROGRESS.md changelog **2026-05-20**.

- **Earlier (this session continuation, 2026-05-18, latest, evening — preserved as historical)**: **`chore:` close T7 (Astro 404 page) + T8 (SW-aware Playwright spec) — two infra-hardening follow-ups in a single ship.** Picked from the post-Number-Friends queue (T6/T7/T8/T9 + a fourth feature game). T7 + T8 won on cost-benefit: both ~30 min, both close real gaps, both fit the same "infra-hardening session" arc as this morning's T2.1 closure. T9 still deferred until v1 retention validation (the actual 3yo user). T6 left for later — open-ended, case strengthens with each ship but no clear target shape until parent feedback arrives. A fourth feature game (Number Bond Pop — earmarked at the More Friends ship) was deferred because the missing-addend partitioning mechanic is age 4+ which doesn't match the 3yo target user; the existing triad covers the full age-3 cardinality skill set. **T7 — `src/pages/404.astro`:** friendly "Page Not Found" landing for any unmatched URL. Replaces the default unstyled "Site not found · GitHub Pages" GH Pages serves when `dist/404.html` is missing. Port of the vanilla `kids-learning-games/404.html` with three small upgrades the vanilla page lacked: (a) Astro `import.meta.env.BASE_URL` template literal for the home link so it relocates automatically if `BASE` ever flips; (b) FOUC-safe dark-mode pre-paint script (same `is:inline` pattern `index.astro` uses) so a dark-mode user doesn't flash the bright purple gradient before the dark-mode bg paints; (c) `body.dark-mode` override styles the vanilla page entirely lacked. Same visual language as `public/offline.html` and the home hero (gradient body, glass-morphism box, big emoji + headline + paragraph + single CTA) so all three "fallback" pages of the site read as one visual family. Single CTA is the right shape for a 3yo: parents read the headline, kid taps the obvious 🏠 Go Home button. **Deliberately NOT included:** GameNav/SettingsModal/BuildInfo (one obvious affordance, mirrors the vanilla design philosophy), `global.css` import (~1 KB inline self-contained CSS so the page renders even if a global stylesheet ever becomes unavailable — resilience > deduplication), SW registration (404 is a terminal page; "Go Home" lands on `index.astro` which registers the SW for any session that hasn't yet, keeps 404 payload minimal, no risk of the 404 page itself becoming a precache target by accident). **Offline-vs-404 interaction documented in 404.astro's header:** when a user with the SW installed navigates to a wrong URL while offline, `setCatchHandler` in `service-worker.ts` serves the precached `offline` page rather than this 404 page (request fails before GH Pages can return `404.html`); that's the right behaviour because "you're offline" is a more actionable message than "page not found" when both are true. Online + wrong URL → 404 page; offline + wrong URL → offline page. **T7 test coverage** (`tests/not-found.spec.ts`, 1 test): direct navigation to `<base>/404.html` returns HTTP 200 with the friendly content (title `Page Not Found`, h1 + paragraph markers, `a.home` href = `/kids-learning-games-astro/`, `Go Home` link text). The "missing-path → 404" *fallback* behaviour is deliberately NOT tested against `astro preview` because it's a property of the static-hosting layer (GH Pages in production, vite preview in CI) and asserting it would couple the suite to upstream preview behaviour with a flaky-CI risk that has no upside; verified manually post-deploy on the live URL. **T8 — `tests/sw.spec.ts`:** the only test file in the suite that opts in to `serviceWorkers: 'allow'` via `test.use({})`; every other suite inherits the global `'block'` default. Closes the gap that on 2026-05-12 the morning Phase-2 SW-install fix unmasked the latent `NavigationRoute(createHandlerBoundToURL('offline'))` bug that served the offline page on every navigation — the bug went undetected because the existing Playwright suite blocks SWs (correct for those tests, but it left the SW itself untested by the consolidated deploy gate). With T2.1 (the hard deploy gate) closed earlier this same session, T8 ensures that gate actually catches what it should. **Four assertions, four failure modes covered:** (1) **SW installs and takes control** — visit home, assert `navigator.serviceWorker.controller !== null` after `waitForFunction` resolves, assert the controller's `scriptURL` matches `/kids-learning-games-astro/service-worker.js` (catches the 2026-05-12 `sw.js` → `service-worker.js` rename regression class). (2) **Workbox precache cache exists with non-trivial entry count** — open `caches.keys()`, find the `*precache*` cache, count entries, assert ≥20 (catches "SW installs but precache manifest is empty / errors" regressions e.g. `injectManifest.globPatterns` misconfig). (3) **Online navigation serves the real page, NOT the offline fallback** (the explicit May-12 NavigationRoute regression test) — visit home, reload (so the navigation goes through the SW), assert title doesn't match `/Offline/` and body doesn't contain `"You're Offline"`; then navigate to Counting Friends, then to Number Friends; assert the same on each. The May-12 bug specifically affected EVERY navigation, not just one path, so testing across multiple pages locks the assertion in. (4) **Offline navigation to an UNCACHED URL serves the offline fallback** — install SW online, flip `context.setOffline(true)`, navigate to a deliberately-missing path, assert the offline page renders. Exercises `setCatchHandler` and proves the offline-fallback path the May-12 hotfix replaced `NavigationRoute` with. **Robust against `astro preview` quirks** — the SW intercepts the navigation BEFORE the preview server sees it (precache miss → network attempt → network fails → setCatchHandler fires), so this assertion doesn't depend on whether vite preview serves dist/404.html for missing paths. (5) **Offline navigation to a CACHED URL serves the real page** (precache works without network — the actual offline-PWA promise) — visit a game online so it precaches, flip offline, navigate to it again, assert the game's content renders not the offline page; catches "precache present but route handler isn't actually serving from it" regressions. **Test isolation:** Playwright spawns a fresh browser context per test by default; with `serviceWorkers: 'allow'` the SW state lives in the context's storage not the browser session, so each test starts clean by construction. An earlier draft had a `beforeEach` cleanup that explicitly unregistered + cleared caches + waited for `!navigator.serviceWorker.controller`; that was REMOVED because it introduces a race — `unregister()` removes the registration but the existing controller stays until next navigation, so a `waitForFunction(!controller)` could hang if the SW wins the install race before the cleanup script lands. Removed; relying on fresh contexts is cleaner and faster. **Why a separate file:** the global `serviceWorkers: 'block'` config sets the default for the whole `tests/` tree; this file flips it via `test.use({})` which is per-file scope. Mixing SW-allow tests into an SW-block file would either require interleaved `test.use({})` calls (fragile) or duplicate file-scope tests (wasteful). Cleaner to give SW-aware tests their own module. **Updated `playwright.config.ts` docstring** points the next reader from the global `serviceWorkers: 'block'` to the `sw.spec.ts` opt-in with a one-line rationale. **Files:** *new* `src/pages/404.astro` (~190 LoC including header doc), `tests/not-found.spec.ts` (~75 LoC), `tests/sw.spec.ts` (~200 LoC); *changed* `playwright.config.ts` (docstring on `serviceWorkers: 'block'` adds T8 pointer). **Verifications:** `npm run check` 0/0/0 across **52 Astro files** (+1 from the 51 baseline — `404.astro`); `npm run build` emits **18 pages** (+1) with `dist/404.html` at 3.1 KB; precache **73 entries** (+2 — `dist/404.html` itself + small page-specific JS chunk for the inline `initSettings` import). The 404 precache key is `"404"` (no leading slash, no `.html`) — same convention as `"offline"`, verified via `grep -oE '"404"|"offline"' dist/service-worker.js` returning both keys. **Standalone follow-up queue drops to 2** — T6 (Stats refactor) and T9 (MP3 narration, deferred until v1 retention). T2.1 + T7 + T8 all closed this session. Full ADR under PROGRESS.md changelog **2026-05-18 (latest, evening)**.

- **Earlier this session (2026-05-18, evening, mid)**: **`feat(numberfriends):` Number Friends — third feature-driven game (preschool numeral recognition for ages 3–4, completes the cardinality triad with Counting Friends + More Friends).** Triggered by the natural follow-up to the More Friends + post-push test-fix iteration earlier this same session — with the hard deploy gate proven to catch regressions and two preschool-math games in production, the obvious next move was either a third feature game or one of the standalone follow-ups (T6/T7/T8/T9 — T9 explicitly deferred until v1 retention). The third-game move wins on user value: numeral recognition is the missing third side of the cardinality triangle. Counting Friends asks the child to count a set and produce a numeral; More Friends asks the child to compare two sets; Number Friends asks the inverse — "show me three" — which requires *recognising* the numeral as a symbol AND *translating* it back to a quantity. 8 rounds per session, one numeral target 2–5 displayed at the top of the stage in a five-frame card, three group panels with distinct sizes below where exactly one matches the target; the child taps the matching group; errorless wrong-tap flow narrates a guided count of the *tapped wrong* group ("four ducks — that was four, not three"), pauses, then narrates a guided count of the *correct* group ("look — one, two, three — three ducks"), and reveals the correct panel with a pulsing ring. Same 4 themes as Counting Friends + More Friends (Pond/Orchard/Sea/Garden), audio narration via Web Speech API with always-visible caption fallback, no scoring/timers/failures. **Two refactor decisions:** (1) The `StageLayout` carve was deferred a *third* time per the explicit cost-benefit analysis now locked in `StoryLayout.astro`'s JSDoc (chrome differences still amount to a body-class rename, hypothetical StageLayout would re-import the entire shell rather than dedupe it, migrating 3 CSS files from `body.story[...]` to `body.stage[...]` is mechanical churn for zero behaviour change). Trigger conditions for actually carving documented in the JSDoc: a fourth stage-game ships AND a meaningful chrome difference emerges; OR the theme union grows to ≥6 keys; OR a stage-only style primitive emerges. (2) The *content* primitives carved at the More Friends ship — `src/lib/preschool-themes.ts` — got their third consumer with **zero changes**, validating the carve as the right level of abstraction. **Files:** *new* `src/data/numberfriends.ts` (~250 LoC), `src/styles/numberfriends.css` (~340 LoC), `src/pages/games/number-friends-game.astro` (~420 LoC), `tests/numberfriends.spec.ts` (~150 LoC); *changed* `src/layouts/StoryLayout.astro` (theme union widened to include `'numberfriends'` + matching pre-dark FOUC + JSDoc rewrite documenting the third StageLayout-carve deferral with explicit re-evaluation conditions), `src/components/GameNav.astro` (+1 link labelled "Find" — verb-based shortening because there's already a "Numbers" link for the existing 1–10 grid game; using the same label twice would make the nav bar ambiguous), `src/pages/index.astro` (+1 home card). **Test-fix lessons applied prophylactically:** (a) the test suite uses `href`-based home-card selector from day one (`a.home-card[href*="number-friends-game"]`) — not retroactively switched to it like addition.spec.ts and comparison.spec.ts had to be after the Counting/More naming overlap collision earlier this session; the Number Friends home-card description references both Counting Friends + More Friends + the cardinality triad explicitly, so a hasText filter would overmatch even more aggressively here. (b) the `narrate()` watchdog is in place from line 1 (same shape as Counting Friends + More Friends — silent-mode `setTimeout(onEnd, 600)` for the no-TTS path, length-based watchdog for headless CI). (c) the `readSSRRound()` anti-race fix is in place from line 1 (kickoff handler reads round 0 from SSR'd DOM rather than re-rendering, preventing the kickoff-vs-tap race that bit Counting Friends 3 sessions back). (d) the `sound: false` `beforeEach` is in place from line 1. **Verifications:** `npm run check` 0/0/0 across **51 Astro files** (+2 from More Friends's 49 baseline); `npm run build` emits **17 pages** (+1) with `dist/games/number-friends-game.html` at 13.7 KB; precache **71 entries** (+3 — page HTML + page-specific JS chunk + new numberfriends.css; the existing `preschool-themes` shared chunk gains its third consumer with no re-emit, verifiable via `grep -l preschool-themes dist/games/*.html | wc -l` returning 3); SSR markup verified via grep (data-theme="numberfriends", data-scene="orchard", three groups with `data-i ∈ {0,1,2}` and `data-size ∈ {1,2,4}` — the deterministic seed `() => 0.42` produces target=2 with mixed decoys [1,4], data-target="2" with exactly 2 filled `nf-target-dot--filled` dots, 7 total `.nf-item` spans = sum of sizes 1+2+4 ✓). **Local Playwright was again blocked by Zscaler** on this dev box (corp proxy intercepts every port and 403s before reaching `astro preview`); the suite was authored against the documented patterns from More Friends's `comparison.spec.ts` and validated via the consolidated CI gate. **Standalone follow-up queue stays at 4** — T6 (Stats panel refactor — case strengthens further now that 3 preschool-math games have separate stats schemas), T7 (404 page port), T8 (SW-aware Playwright spec), T9 (pre-recorded MP3 narration; was Counting+More-only, now naturally scopes to *all three* preschool-math games). Full ADR under PROGRESS.md changelog **2026-05-18 (latest)**.

- **Earlier this session (2026-05-18, later)**: **`fix(tests):` switch home-card assertions to href-based selectors** (`56212d5`). The `feat(comparison)` push (`cde2833`) + the docs follow-up (`8325ede`) landed locally green but CI went red within ~3 min of the push: the new More Friends home-card description ("Companion to Counting Friends — …") collided with the existing `tests/addition.spec.ts` home-page test that filtered cards by `hasText: 'Counting Friends'` — the substring filter now matched 2 cards (the Counting Friends card AND the More Friends card whose description mentions it), so `toHaveCount(1)` failed and the consolidated deploy gate blocked the deploy. Without GitHub API access (Zscaler 403s authenticated requests on this dev box), the only diagnostic was reasoning about likely failure modes from the source. Fixed in `56212d5` by switching both home-card assertions (addition + comparison) to href-based selectors (`a.home-card[href*="counting-friends-game"]` / `…magnitude-comparison-game`) — unique by construction, stable against future cards mentioning sibling-game titles in copy. CI validated the fix on the next iteration: both badges back to `passing`, deploy completed, live URL serves the new game (HTTP 200, all 5 expected SSR markers present). **The hard deploy gate (T2.1 closed earlier this session) worked exactly as designed** — it caught the test regression from a feature push and blocked a half-broken deploy from going live, exactly the failure mode T2.1 was filed to prevent (the precedent being the Counting Friends `1a66542` ship 3 sessions ago that deployed green while tests were red, exposing users to a half-broken game). **Lesson:** when adding a kid-game whose description references a sibling game, prefer href-based home-card selectors over hasText. Pattern now established in both preschool-math suites.

- **Earlier this same session (2026-05-18, mid-session)**: **`feat(comparison):` More Friends — second feature-driven game (preschool magnitude comparison for ages 3–4, sister/precursor to Counting Friends).** Triggered by the natural follow-up to Counting Friends — magnitude comparison ("which has more?") is the developmental precursor to addition, so for the actual 3yo user this game is *easier* than Counting Friends and reinforces what Counting Friends teaches. 8 rounds per session, two side-by-side groups of identical themed objects (sizes 1–4 each, always unequal); the child taps the bigger group; errorless flow on a wrong tap narrates a guided count of *both* groups (one through left-side count, then right-side count) then reveals the correct side with a pulsing ring. Same 4 themes as Counting Friends (Pond/Orchard/Sea/Garden), audio narration via Web Speech API with always-visible caption fallback, no scoring/timers/failures. **Two refactors landed at this ship:** (1) The `StageLayout` carve was deferred a second time per rule #5 — actual chrome differences between StoryLayout and a hypothetical StageLayout amount to a body-class rename only; both games already scope every class under `body.story[data-theme='X']`. Carving a near-identical sister layout would add code without removing any. (2) The *content* primitives that DID get extracted: `src/lib/preschool-themes.ts` (95 LoC) carves the 4-theme catalog + `ThemeMeta` + `THEMES` + `THEME_BY_KEY` + `numberWord` / `cap` / `nounFor` narration helpers from the previous inline-in-`addition.ts` definitions; `addition.ts` re-exports from the lib so Counting Friends's page imports unchanged (`AdditionTheme = PreschoolTheme` backward-compat alias). **Files:** *new* `src/lib/preschool-themes.ts` (95 LoC), `src/data/comparison.ts` (220 LoC), `src/styles/comparison.css` (385 LoC), `src/pages/games/magnitude-comparison-game.astro` (470 LoC), `tests/comparison.spec.ts` (140 LoC); *changed* `src/data/addition.ts` (themes moved to lib), `src/layouts/StoryLayout.astro` (theme union widened + pre-dark FOUC + JSDoc rewrite explaining why StageLayout is still deferred), `src/components/GameNav.astro` (+1 link), `src/pages/index.astro` (+1 home card). **Verifications:** `npm run check` 0/0/0 across **49 Astro files** (+3 from the 46 baseline); `npm run build` emits **16 pages** (+1) with `dist/games/magnitude-comparison-game.html` at 12.7 KB; precache **68 entries** (+4); SSR markup verified (data-theme="comparison", mfStage/mfGroupLeft/mfGroupRight/mf-vs/mfCaption all present, 4 mf-item occurrences match the seed-0.42 (1, 3) orchard round, home card emits `href=".../games/magnitude-comparison-game"`). **Local Playwright was blocked by Zscaler** on this dev box (corp proxy intercepts every port and 403s before reaching `astro preview`); the suite was authored against the documented patterns from Counting Friends's `addition.spec.ts` (with the prophylactic `readSSRRound()` + `narrate()` watchdog + `sound: false` `beforeEach` already in place) and validated via the consolidated CI gate. **Standalone follow-up queue stays at 4** — T6 (Stats panel refactor — case strengthens now that 2 preschool-math games have separate stats schemas), T7 (404 page port), T8 (SW-aware Playwright spec), T9 (pre-recorded MP3 narration; was Counting-Friends-only, now naturally scopes to *both* preschool-math games). Full ADR under PROGRESS.md changelog **2026-05-18 (still later)**.
